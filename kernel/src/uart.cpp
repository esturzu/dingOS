...
public:
  ...
  {
    volatile uint32_t* data_register = (volatile uint32_t*) (base_address + data_register_offset);
    *data_register = data;
  }

  inline void set_integer_braud(uint16_t integer_braud)
  {
    volatile uint32_t* integer_braud_register = (volatile uint32_t*) (base_address + integer_braud_offset);
    *integer_braud_register = integer_braud;
  }

  inline void set_fractional_braud(uint8_t fractional_braud)
  {
    volatile uint32_t* fractional_braud_register = (volatile uint32_t*) (base_address + fractional_braud_offset);
    *fractional_braud_register = fractional_braud;
  }

  inline void set_line_control(uint8_t line_control)
  {
    volatile uint32_t* line_control_register = (volatile uint32_t*) (base_address + line_control_offset);
    *line_control_register = line_control;
  }

  inline void set_control(uint16_t control)
  {
    volatile uint32_t* control_register = (volatile uint32_t*) (base_address + control_offset);
    *control_register = control;
  }

  inline void clear_interrupts()
  {
    volatile uint32_t* interrupt_clear_register = (volatile uint32_t*) (base_address + interrupt_clear_offset);
    *interrupt_clear_register = 0b11111111111;
  }

  inline void set_interrupt_mask(uint16_t mask)
  {
    volatile uint32_t* interrupt_mask_set_clear_register = (volatile uint32_t*) (base_address + interrupt_mask_offset);
    *interrupt_mask_set_clear_register = mask;
  }

  inline bool transmit_fifo_full ()
  {
    volatile uint32_t* flag_register = (volatile uint32_t*) (base_address + flag_register_offset);
